...
>  从[提问指南](https://lug.ustc.edu.cn/wiki/doc/howtoask/)转载 来自中国科学技术大学 Linux 用户协会 对于一些链接进行修改
>
> 遵循 CC-BY-NC-SA 4.0
...
- 例 1：镜像站怎么挂了
  ...
  ```
  Copy codeE: 无法下载 http://mirrors.ustc.edu.cn/ubuntu/pool/main/g/gcc-11/g%2b%2b-11_11.3.0-1ubuntu%7e22.04_amd64.deb  404  Not Found [IP: 202.38.95.110 443]
...
```
Copy code为什么我运行 apt update 会输出这个：
...
```
Copy code为什么我运行 apt update 会输出这个：E: Could not open lock file /var/lib/apt/lists/lock - open (13: Permission denied)
```

接下来可以用上述简单策略来处理查询：

```
Copy codeapt update 输出 E: Could not open lock file /var/lib/apt/lists/lock - open (13: Permission denied)
```
...
自 2023 年初大语言模型（GPT 等）逐渐普及以来，询问大语言模型已成为解决问题的有效途径，而且比真人更有耐心。不过这同样需要技巧，请参考本文「[附录：如何正确向大语言模型提问](https://lug.ustc.edu.cn/wiki/doc/howtoask/#llm)」。
...
- 例子 1：获取字符串后三位字符？
  ...
  ```
  Copy codeQ: 我怎么用 Shell 取得一个字符串的后 3 位字符？
...
- 我们补充另一个真实的例子（经过大幅简化）：
  ...
  ```
  Copy codeQ: 有没有人在导入的时候遇到 TypeError: Can't create an SSLContext object without an ssl module? 的报错？
  A1: 你编译的时候没加 ssl 模块吗
...
```
Copy codeA (8:00): Hi，我遇到了 XXXXXXX 错误，然后 YYYY 命令跑不了，怎么办？
...
```
Copy code我遇到了一个问题，我的 C++ 程序需要运行一个 CUDA kernel，我要怎么写 cmake 来编译这个 kernel 并且让我的程序调用？
```
...
```
Copy code我的 vulkan 程序在 wayland 下没有显示出来，托盘那里有程序的图标，怎么回事？
```
...
- 不美观
- 因为[莫尔纹](https://en.wikipedia.org/wiki/Moiré_pattern)现象，字/图像可能会拍不清楚
- 其他人可能得歪着头才能看清内容
- 不利于其他人搜索类似问题
...
````
Copy codeHuman: 怎么在命令行里面下载一个 url？
...
````
Copy codeHuman: 在 Windows Powershell 下，怎么在命令行里面下载一个 url？直接给命令即可。
